...
namespace trabajo.Controllers
{
    ...
    public class AdministradorController : Controller
    {
        public async Task<IActionResult> Analistas()
        {
            ...
            if (analista == null)
            {
                var modeloVacio = new AdminAnalistaViewModel
                {
                    IdAdministrador = administrador.Id,
                    IdAnalista = 0,
                    NombreAnalista = "Sin analista asignado",
                    DniAnalista = "",
                    CorreoAnalista = "",
                    CelularAnalista = "",
                    EstadoActivo = false,
            ...
            DateTime limiteActivoAnalista = DateTime.Now.AddMinutes(-2);

            bool analistaConectado =
                analista.EstadoActivo == true &&
                analista.UltimaConexion != null &&
                analista.UltimaConexion >= limiteActivoAnalista;

            var solicitudes = await _context.SOLICITUD_CREDITO
                .AsNoTracking()
                .Include(x => x.USUARIO)
            ...
            int solicitudesAsignadas = solicitudes.Count;

            int solicitudesCompletadas = solicitudes.Count(x =>
                x.Estado == "Aprobado" ||
                x.Estado == "Rechazado" ||
                x.Estado == "Cancelado");

            int solicitudesEnProceso = solicitudes.Count(x =>
                x.Estado == "En Evaluación");

            int solicitudesPendientes = solicitudes.Count(x =>
                x.Estado == "Pendiente");

            int tasaEfectividad = solicitudesAsignadas == 0
                ? 0
                : (int)Math.Round((double)solicitudesCompletadas * 100 / solicitudesAsignadas);

            var actividades = new List<AdminAnalistaActividadViewModel>();

            foreach (var solicitud in solicitudes)
            {
                actividades.Add(new AdminAnalistaActividadViewModel
                {
                    Titulo = $"Solicitud registrada #{solicitud.NumeroSolicitud}",
                    Descripcion = $"Cliente: {solicitud.USUARIO?.Nombre} {solicitud.USUARIO?.Apellido}",
                ...
                if (solicitud.HISTORIAL_ESTADOS != null)
                {
                    foreach (var historial in solicitud.HISTORIAL_ESTADOS)
                    {
                        actividades.Add(new AdminAnalistaActividadViewModel
                        {
                            Titulo = $"Solicitud #{solicitud.NumeroSolicitud} - {historial.EstadoActual}",
                            Descripcion = $"Cliente: {solicitud.USUARIO?.Nombre} {solicitud.USUARIO?.Apellido}",
            ...
            var mensajes = await _context.MENSAJE_ADMIN_ANALISTA
                .AsNoTracking()
                .Where(x =>
            ...
            var modelo = new AdminAnalistaViewModel
            {
                ...
                NombreAnalista = $"{analista.Nombre} {analista.Apellido}",
                DniAnalista = analista.Dni,
                CorreoAnalista = analista.Correo,
                CelularAnalista = analista.Celular,
                EstadoActivo = analistaConectado,
                FechaAsignacion = analista.FechaRegistro,
        ...
        [HttpGet]
        public async Task<JsonResult> EstadoAnalista(int idAnalista)
        {
            ...
            bool conectado =
                analista.EstadoActivo == true &&
                analista.UltimaConexion != null &&
                analista.UltimaConexion >= limiteActivo;

            return Json(new
            {
                conectado = conectado,
                texto = conectado ? "Activo" : "Inactivo",
                ultimaConexion = analista.UltimaConexion == null
                    ? "Sin conexión"
                    : analista.UltimaConexion.Value.ToString("dd/MM/yyyy hh:mm tt")
            });
        }
    ...
    .ToList();
     ...
     .ToList();
            ...
            int pagosCompletadosMes = (
                ...
                join solicitud in _context.SOLICITUD_CREDITO on cuota.SOLICITUD_CREDITO_Id_Solicitud equals solicitud.Id_Solicitud
                where solicitud.Estado == "Aprobado"
                      && solicitud.FechaSolicitud >= primerDiaDelMes
                      && solicitud.FechaSolicitud < primerDiaSiguienteMes
